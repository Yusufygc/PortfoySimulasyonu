...
def _optimize_min_volatility_weights(
    mean_returns: np.ndarray,
    cov_matrix: np.ndarray,
    max_single_weight: float,
    remaining_weight: float,
) -> np.ndarray:
    """`_optimize_valid_weights()` ile aynı kısıt/başlangıç noktası, farklı hedef:
    Sharpe yerine doğrudan volatiliteyi minimize eder — Verimli Sınır (Efficient
    Frontier) grafiğinin 3. noktası (minimum risk portföyü) için (bkz. plan §7.3
    madde 5). `mean_returns` imza uyumluluğu için alınır, hesaba katılmaz."""
    num_assets = len(mean_returns)
    initial_weights = np.array([1.0 / num_assets] * num_assets)
    constraints = {"type": "eq", "fun": lambda weights: np.sum(weights) - 1}
    max_weight = max(max_single_weight, 1.0 / num_assets)
    ...
    result = minimize(
        lambda weights: _portfolio_volatility(weights, cov_matrix),
        initial_weights,
        method="SLSQP",
        bounds=bounds,
        constraints=constraints,
    ...
    if not result.success:
        raise ValueError("Minimum volatilite portföyü hesaplanamadi. Lutfen tekrar deneyin.")
...
class OptimizationService:
    ...
    def policy(self) -> OptimizationPolicy:
        """Varsayılan (DI singleton) politika — per-call override üretmek isteyenler için (bkz. §5.3)."""
        return self._policy

    def optimize_dashboard_portfolio(self, policy: Optional[OptimizationPolicy] = None) -> OptimizationResult:
        ...
        return self._optimize(tickers, current_weights, policy=policy)
    ...
    def _optimize(
        ...
        """policy verilirse (bkz. RiskOptimizationBridgeService, §5.3) yalnızca bu çağrı için
        geçerlidir — DI singleton'ın varsayılan politikası (`self._policy`) değişmez."""
        ...
        mean_returns, cov_matrix = _build_return_model(price_df, effective_policy.trading_days_per_year)
        ...
        optimal_weights_scaled = _optimize_valid_weights(
            mean_returns.values,
            cov_matrix.values,
            effective_policy.risk_free_rate,
            effective_policy.max_single_weight,
            remaining_weight,
        )
        ...
        current_metrics = self._calculate_metrics(
            current_valid_weights, mean_returns.values, cov_matrix.values, risk_free_rate=effective_policy.risk_free_rate,
        )
        optimized_metrics = self._calculate_metrics(
            optimal_weights_scaled, mean_returns.values, cov_matrix.values, risk_free_rate=effective_policy.risk_free_rate,
        )
        min_volatility_weights_scaled = _optimize_min_volatility_weights(
            mean_returns.values,
            cov_matrix.values,
            effective_policy.max_single_weight,
            remaining_weight,
        )
        min_volatility_metrics = self._calculate_metrics(
            min_volatility_weights_scaled, mean_returns.values, cov_matrix.values, risk_free_rate=effective_policy.risk_free_rate,
        )
        ...
        return OptimizationResult(
            ...
            min_volatility_metrics=min_volatility_metrics,
        )
    ...
    def _calculate_metrics(
        ...
        rf = risk_free_rate if risk_free_rate is not None else self._risk_free_rate
