...
def _build_cost_data(trades) -> dict:
    cost_data: dict = defaultdict(lambda: {"qty": 0, "cost": Decimal("0")})
    for trade in trades:
...
def _position_details_dict(stock_id: int, quantity: int, stock, total_cost: Decimal, current_price: "Optional[Decimal]") -> dict:
...
class ModelPortfolioSnapshotService:
    ...
    def get_positions_with_details(
        ...
        if not positions:
            return []
        stocks = self._stock_repo.get_stocks_by_ids(list(positions.keys()))
        stock_map = {stock.id: stock for stock in stocks}
        cost_data = _build_cost_data(trades)
        result = []
        for stock_id, quantity in positions.items():
            current_price = price_map.get(stock_id) if price_map else None
            result.append(_position_details_dict(
                stock_id, quantity, stock_map.get(stock_id),
                cost_data[stock_id]["cost"], current_price,
            ))
        return result
