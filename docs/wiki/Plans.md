...
### 1. google-genai SDK Göçü
* **Gerekçe**: Gemini API entegrasyonunda kullanılan `google.generativeai` paketinin desteği sonlanmıştır. Pytest çıktılarında bu duruma dair `FutureWarning` uyarısı verilmektedir.
* **Aksiyon**: [gemini_service.py](file:///d:/1KodCalismalari/Projeler/VIBE_CODING_UYGULAMA_DENEMELERI/Merge_PortfoySim/PortfoySimulasyonu/src/ui/pages/ai_page/core/gemini_service.py) dosyasını yeni nesil resmi `google.genai` SDK'sına göç ettirmek.

### 2. Dinamik Risk Profili Optimizasyon Entegrasyonu
...
### 3. Çoklu Pencere (Multi-Window / Docking) Desteği
